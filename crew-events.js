async function brawl() {
	let killers = filterEventActors(crew).filter(pirate => roll(pirate) + skillValue([pirate], skill.swashbucklin));
	...
	while(killers.length > 1) {
		...
		killers = killers.filter(p => crew.includes(p));
		availableVictims = availableVictims.filter(p => crew.includes(p));
	}
}
...
const crewEventTable = [
	{
		...
		handler: async () => {
			let overboard = filterEventTargets(crew).filter(pirate => roll(pirate) + skillValue([pirate], skill.swaggerin) == 1);
	...
	},
	{
		...
		handler: async () => {
			let lousy = filterEventTargets(crew).filter(pirate => roll(pirate) < 3);
			...
			if(!lousy.length) {
				return { continueText:'Praise crew’s hygiene', description:'Everyone avoids the lice.' };
			}
		}
	},
	{
		...
		handler: async () => {
			...
			if(stealers.length) {
				...
				return { continueText:'Make do without their help', description: `${getPirateName(boxedPirate)} is accused of cheating and locked in a chest to be released when we return to port. They are unable to do anything but swear loudly from the chest but also require no grog, being forced to drink seawater.` };
			}
	...
	},
	{
		...
		handler: async () => {
			...
			if(sortedByJigSkill.length > 1 && sortedByJigSkill[0].result == sortedByJigSkill[1].result) {
				await brawl();
				return { continueText:'Avoid future jig contests', description:'The contest ends in a draw, and the crew start brawling.' };
			} else {
	...
	},
	{
		...
		handler: async () => {
			let all = filterEventActors(filterEventTargets(crew));
			let allOkay = true;
			...
			if(allOkay) {
				return { continueText:'How fortunate', description:'Everyone is tolerating the voyage.' };
			}
		}
	},
	{
		...
		handler: async () => {
			if(roll() + skillValue(filterEventActors(crew), skill.swaggerin) > 3) {
		...
		handler: async () => {
			...
			if(victim) {
				if(roll(victim) < 4) {
					...
					return { continueText:'Commend the successful hat heist', description:`The bravery of ${getPirateName(thief)} shocks ${getPirateName(victim)}, who loses their Swaggerin’ Skill. However, ${getPirateName(thief)} gains the Swaggerin’ Skill along with their new hat.` };
				} else {
					await kill(thief, `${getPirateName(victim)} was shot by ${getPirateName(victim)} in retaliation for hat theft`, true);
					return { continueText:'Leave this hat alone', description:`${getPirateName(victim)} will not stand for such disrespect and shoots ${getPirateName(victim)} dead!` };
				}
			}
		}
	},
	...
	{
		name: 'The Box',
		description: 'There has been a general ruckus on the deck of the ship, and the captain feels some pirates need some time in The Box, until the ship next reaches port. For this time they require no grog, only water, but cannot do anything other than stay in their box.',
		continueText: 'Get boxing',
		handler: async () => {
			let boxed = filterEventTargets(crew).filter(pirate => roll(pirate) == 1);
	...
	},
	{
		...
		handler: async () => {
			...
			if(!dismembered.length) {
				...
				return { continueText:'Suffer their complaining', description:`${getPirateName(choice)} has a stump which became gangrenous. They cannot do anything but complain about until they return to port.` };
			}
		}
	},
	{
	...
	},
	{
		...
		handler: async () => {
			var soberPirates = filterEventActors(crew).filter(pirate => !pirate.captain);
	...
	},
	{
		...
		handler: async () => {
			let deserters = filterEventActors(filterEventTargets(crew)).filter(pirate => roll(pirate) == 1);
			for(let pirate of deserters) {
				if(!crew.includes(pirate)) {
					continue;
				}
				incrementBooty(-5);
				await kill(pirate, 'abandoned the crew after stealing some of their booty', false);
			}
		}
	},
	{
	...
	},
	{
		...
		handler: async () => {
			...
			if(pirate) {
				...
				return { continueText:'Let them have the week off work', description:`${getPirateName(pirate)} got a nasty snip on their hand.` };
			}
		}
	},
	{
	...
	},
	{
		...
		handler: async () => {
			...
			if(winner) {
				...
				return { continueText:'Admire their new look', description:`A brawl broke out over the dress. ${getPirateName(winner.pirate)} won and will keep the dress forever.` };
			}
		}
	},
];